(function() {
	var ui_class = {
		...
		get_vendor: function()
		{
			...
			let vendorname = "SIFLOWER";
			$.ajax(L.globals.resource + `/vendor/${vendorname}/config.json`,{
				cache:    true,
				dataType: 'text',
				success:  function(data) {
					L.globals.vendor = JSON.parse(data);
					L.globals.vendorname = vendorname;
					deferred.resolve();
				}
			});
			return deferred;
		...
		login: function(invalid)
		{
			...
			$.ajax(L.globals.resource + '/template/login.htm', {
				...
				success:  function(data) {
					...
					let vendorinfo = L.globals.vendor;

					if(L.globals.vendorname && vendorinfo.logo){
						$(".logo img").attr("src",`../../luci2/vendor/${L.globals.vendorname}/images/${vendorinfo.logo}`);
					}

					if(L.globals.vendorname && vendorinfo.QRcode){
						$(".qrcode img").attr("src",`../../luci2/vendor/${L.globals.vendorname}/images/${vendorinfo.QRcode}`);
					}else{
						$(".qrcode").hide();
					}

					// 登录页主体背景颜色
					if(vendorinfo.mainColor  == "1"){
						$(".custombgcolor").css("background-color","#3e62ad");
					}else if(vendorinfo.mainColor  == "2"){
						$(".custombgcolor").css("background-color","#f8bb06");
					}else if(vendorinfo.mainColor  == "3"){
						$(".custombgcolor").css("background-color","#5b92f7");
					}

					if(L.g_language === "en"){
						$("#login_error").find("span").text("Wrong password!");
						$("#forgetpwd_page_link").text("Forget password?");
						$("#forgetpwd_page_tips").text("If you forget the password, please press and hold the reset button to restore");
						$(".log_til").text("Login");
						if(vendorinfo.abbreviation_en){
							$(".logo>div").text(vendorinfo.abbreviation_en);
						}else{
							$(".logo>div").text("AC1200 Router");
						}

						if(vendorinfo.maintitle_en){
							$(".main_title").text(vendorinfo.maintitle_en);
						}else{
							$(".main_title").text("Welcome to AC1200 Router");
						}

						if(vendorinfo.subtitle_en){
							$(".sub_title").text(vendorinfo.subtitle_en);
						}else{
							$(".sub_title").text("Dual-band concurrent full Gigabit Wi-Fi 5 routing chip");
						}

						if(vendorinfo.abbreviation_en){
							$(".qrcode>div").text(`Scan the code to follow${vendorinfo.abbreviation_en}official account`);
						}

						if(vendorinfo.feature[0].title_en){
							$(".product_features").find(".product_title").eq(0).text(vendorinfo.feature[0].title_en);
						}else{
							$(".product_features").find(".product_title").eq(0).text("High integration");
						}

						if(vendorinfo.feature[0].describe_en){
							$(".product_features").find(".product_describe").eq(0).text(vendorinfo.feature[0].describe_en);
						}else{
							$(".product_features").find(".product_describe").eq(0).text("The A28 integrates dual-core CPU, 2x2 MIMO 2.4GHz & 5GHz Wi-Fi baseband and MAC, dual-band RF, Balun, built-in power amplifier, low-noise amplifier, filter, transceiver switch, and provides the ultimate single-frequency router for dual-band routers. Chip solutions.");
						}

						if(vendorinfo.feature[1].title_en){
							$(".product_features").find(".product_title").eq(1).text(vendorinfo.feature[1].title_en);
						}else{
							$(".product_features").find(".product_title").eq(1).text("Outstanding performance");
						}

						if(vendorinfo.feature[1].describe_en){
							$(".product_features").find(".product_describe").eq(1).text(vendorinfo.feature[1].describe_en);
						}else{
							$(".product_features").find(".product_describe").eq(1).text("A28 uses dual-core quad processors, the highest frequency is 1GHz, with high-speed WiFi module and hardware forwarding engine, provides stable 866Mbps+300Mbps wireless access and full gigabit routing forwarding performance, supports up to 128 concurrent connections for users, operating temperature range -40℃ To +125℃.");
						}

						if(vendorinfo.feature[2].title_en){
							$(".product_features").find(".product_title").eq(2).text(vendorinfo.feature[2].title_en);
						}else{
							$(".product_features").find(".product_title").eq(2).text("Rich interface");
						}

						if(vendorinfo.feature[2].describe_en){
							$(".product_features").find(".product_describe").eq(2).text(vendorinfo.feature[2].describe_en);
						}else{
							$(".product_features").find(".product_describe").eq(2).text("A28 has rich external interfaces, supports 4 antennas (compatible with 802.11a/b/g/n/ac), supports USB2.0, SPI, UART, I2C, PWM and RGMII and other external interfaces, providing up to dozens GPIO, to meet the requirements of complex application scenarios.");
						}

						if(vendorinfo.corporateName_en){
							$(".footer>div").text(vendorinfo.corporateName_en);
						}else{
							$(".footer>div").text("AC1200 Router");
						}

						$("#userpassword_ctrl").attr("placeholder","Please enter the password");
					}else{
						if(vendorinfo.abbreviation){
							$(".logo>div").text(vendorinfo.abbreviation);
						}
						if(vendorinfo.maintitle){
							$(".main_title").text(vendorinfo.maintitle);
						}
						if(vendorinfo.subtitle){
							$(".sub_title").text(vendorinfo.subtitle);
						}
						if(vendorinfo.abbreviation){
							$(".qrcode>div").text(`扫码关注${vendorinfo.abbreviation}公众号`);
						}
						if(vendorinfo.feature[0].title){
							$(".product_features").find(".product_title").eq(0).text(vendorinfo.feature[0].title);
						}
						if(vendorinfo.feature[0].describe){
							$(".product_features").find(".product_describe").eq(0).text(vendorinfo.feature[0].describe);
						}
						if(vendorinfo.feature[1].title){
							$(".product_features").find(".product_title").eq(1).text(vendorinfo.feature[1].title);
						}
						if(vendorinfo.feature[1].describe){
							$(".product_features").find(".product_describe").eq(1).text(vendorinfo.feature[1].describe);
						}
						if(vendorinfo.feature[2].title){
							$(".product_features").find(".product_title").eq(2).text(vendorinfo.feature[2].title);
						}
						if(vendorinfo.feature[2].describe){
							$(".product_features").find(".product_describe").eq(2).text(vendorinfo.feature[2].describe);
						}
						if(vendorinfo.corporateName){
							$(".footer>div").text(vendorinfo.corporateName);
						}
					}
		...
		initialPage:function(){
			L.session.login('root','admin').then(function(data){
				L.globals.sid = data.ubus_rpc_session;
				L.ui.get_basic_setting('basic_setting', 'web_guide', 'inited').then(function (value) {
					if (value == '0'){
						$.ajax(L.globals.resource + `/template/guide_start.htm`, {
							cache: true,
							dataType: 'text',
							success: function (data) {
								...
								let vendorinfo = L.globals.vendor;
								let vendor_name = L.globals.vendorname

								if(vendor_name && vendorinfo.logo){
									$(".logo img").attr("src",`../../luci2/vendor/${vendor_name}/images/${vendorinfo.logo}`);
								}

								if(L.g_language == "en"){
									if(vendorinfo.abbreviation_en){
										$(".loginname").text(vendorinfo.abbreviation_en);
									}
									if(vendorinfo.corporateName_en){
										$(".footer>div").text(vendorinfo.corporateName_en);
									}
									if(vendorinfo.maintitle_en){
										$(".guide_h").text(vendorinfo.maintitle_en);
									}
								}else{
									if(vendorinfo.abbreviation){
										$(".loginname").text(vendorinfo.abbreviation);
									}
									if(vendorinfo.corporateName){
										$(".footer>div").text(vendorinfo.corporateName);
									}
									if(vendorinfo.maintitle){
										$(".guide_h").text(vendorinfo.maintitle);
									}
								}
		...
		guide: function(name) {
			...
			$.ajax(L.globals.resource + `/template/${name}.htm`, {
				cache: true,
				dataType: 'text',
				success: function (data) {
					data = data.replace(/<%([#:=])?(.+?)%>/g, function(match, p1, p2) {
		...
		callMenuCallback: function(entries){
			if (L.globals.wds || L.globals.bridge) {
				for (const now in entries) {
					...
					if (L.globals.wds) {
						if (
							check === 'advance' ||
							check === 'network' ||
							now === 'wireless/adv_wifi' ||
							now === 'wireless/guest_wifi' ||
							now === 'wireless/visit_control'
						) {
							delete entries[now];
						}
					}

					if (L.globals.bridge && now === 'wireless/guest_wifi') {
						delete entries[now];
					}
				}
			}
			...
			self.renderMainMenu().then(function () {
				...
				$.ajax(L.globals.resource + '/template/theme.htm', {
					...
					success: function (data) {
						...
						if(vendorinfo.mainColor  == "1"){
							$(".custombgcolor").css("background-color","#3e62ad");
							document.documentElement.style.setProperty('--menu-nav-active', "#ffb61e");
						}else if(vendorinfo.mainColor  == "2"){
							$(".custombgcolor").css("background-color","#FFC107");
							document.documentElement.style.setProperty('--menu-nav-active', "#B0B0B0");
						}else if(vendorinfo.mainColor  == "3"){
							$(".custombgcolor").css("background-color","#6198fc");
							document.documentElement.style.setProperty('--menu-nav-active', "#ffd168");
						}
						...
						if(L.g_language == "en"){
							...
							if(vendorinfo.corporateName_en){
								$("#footername").text(vendorinfo.corporateName_en);
							}else{
								$("#footername").text("AC1200 Router")
							}

							$("#logout_btn").text("logout");
							$(".nav_list").find(".nav_title").eq(0).text("HOME");
							$(".nav_list").find(".nav_title").eq(1).text("INTERNET");
							$(".nav_list").find(".nav_title").eq(2).text("Wi-Fi");
							$(".nav_list").find(".nav_title").eq(3).text("DEVICES");
							$(".nav_list").find(".nav_title").eq(4).text("MORE");
						}else{
							$(".logo>div").text(vendorinfo.abbreviation);
							$("#footername").text(vendorinfo.corporateName);
						}
	...
	ui_class.menu = ui_class.AbstractWidget.extend({
		...
		a_renderN: function(i, sid, dt_sid, dd_sid, li_sid) {
                ...
                renderNodes: function(childs, level, min, max)
                {
						...
						// 主体内容背景色自定义配置
						let main_color = L.globals.vendor.mainColor;
						if(main_color == "1"){
							document.documentElement.style.setProperty('--menu-active', "#4870c2");
						}else if(main_color == "2"){
							document.documentElement.style.setProperty('--menu-active', "#FFC107");
						}else if(main_color == "3"){
							document.documentElement.style.setProperty('--menu-active', "#6198fc");
						}
